...
// Read parses all config files with the specified options and returns an iterator which returns all files in the right order.
// It is the callers responsibility to close the file. If an error is returned by the iterator then this mus be treated as fatal
// error an must fail the config file parsing. Expected ENOENT errors are already ignored in this function and does must not be
// handled again by callers.
// The given File must not be nil and populated with valid options.
