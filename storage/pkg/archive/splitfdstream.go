...
// It parses the composefs-rs splitfdstream format and provides tar headers.
...
func (i *splitFDStreamIterator) Next() (*tar.Header, error) {
	...
	if header.Typeflag == tar.TypeReg && header.Size > 0 {
		if err := binary.Read(i.stream, binary.LittleEndian, &prefix); err != nil {
			return nil, fmt.Errorf("failed to read content chunk prefix for %s: %w", header.Name, err)
		}
