...
func createTestBlob(t *testing.T, fileName string, size int) libartTypes.ArtifactBlob {
	t.Helper()
...
func helperAddArtifact(t *testing.T, as *ArtifactStore, refName string, fileNames map[string]int, options *libartTypes.AddOptions) *digest.Digest {
	...
	for fileName, size := range fileNames {
		blobs = append(blobs, createTestBlob(t, fileName, size))
	}
...
func TestArtifactStore_Add_ReplaceNonExistent(t *testing.T) {
	...
	artifactDigest := helperAddArtifact(t, as, refName, fileNames, replaceOptions)
	assert.NotEmpty(t, artifactDigest.String())
...
func TestArtifactStore_Add_ReplaceChangesDigest(t *testing.T) {
	...
	originalDigest := helperAddArtifact(t, as, refName, fileNames, nil)
	...
	replacedDigest := helperAddArtifact(t, as, refName, replaceFileNames, replaceOptions)
